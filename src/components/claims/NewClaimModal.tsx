...
import { createClaimBC } from "../../utils/contractUtils";
...
export default function NewClaimModal(props: NewClaimModalProps) {
  ...
  const form = useForm<Claim>({
    initialValues: {
      policyId:
        user && user.policiesTaken && user?.policiesTaken.length > 0
          ? user?.policiesTaken[0]._id || ""
          : "",
      ...
      <Stack spacing="md">
        <Stack spacing="xs" p="xs">
          <Select
            ...
            data={user.policiesTaken
              .filter((p) => p.status === "APPROVED")
              .map((p) => ({ value: p._id || "", label: p.policy?.title }))}
        ...
        <Stack spacing="xs" p="xs">
          ...
          <DatePicker
            ...
            value={new Date(form.values.receiptDate)}
          />
        </Stack>
        <Divider />
        <Stack spacing="xs" p="xs">
          ...
          <NumberInput
            label="Claim Amount"
            {...form.getInputProps("claimAmount")}
          />
        </Stack>
        <Divider />
        <Stack spacing="xs" p="xs">
              ...
              <Group position="apart">
                ...
                <ActionIcon
                  ...
                  onClick={() =>
                    form.setValues({
                      ...form.values,
                      attachments: form.values.attachments.filter(
                        (x) => x !== a
                      ),
                    })
                  }
